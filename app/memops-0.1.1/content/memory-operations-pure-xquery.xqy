...
declare function mem:insert-child(
...
declare function mem:insert-child-first(
  $transaction-map as map(*),
  $parent-node as element()*,
  $new-nodes as node()*)
as map(*)?
...
declare function mem:insert-before(
  $transaction-map as map(*),
  $sibling as node()*,
  $new-nodes as node()*)
as map(*)?
...
declare function mem:insert-after(
  $transaction-map as map(*),
  $sibling as node()*,
  $new-nodes as node()*)
as map(*)?
...
declare function mem:replace-value(
  $transaction-map as map(*),
...
declare function mem:replace-contents(
  $transaction-map as map(*),
  $nodes-to-change as node()*,
  $contents as node()*)
as map(*)?
...
declare function mem:transform(
  $transaction-map as map(*),
  $nodes-to-change as node()*,
...
declare function mem:copy($node-to-copy as node())
as map(*)
{
...
(: Execute transaction :)
declare function mem:execute($transaction-map as map(*))
as node()*
{
  if (exists(map:get($transaction-map, "nodes-to-modify")))
  then
    mem:safe-copy(mem:process(
      $transaction-map,
    ...
    mem:safe-copy(map:get($transaction-map, "copy"))
};
...
as map(*)
{
  ...
  then 
    (: Creates elements based off of generate-id (i.e., node is 12439f8e4a3, then we get back <mem:_12439f8e4a3/>) :)
    let $modified-node-ids as element()* := $nodes-to-modify ! mem:id-wrapper(.) 
    return
    (
    ...
    map:merge((
      ...
      map:entry(
        "modifier-nodes",
        (<mem:modifier-nodes>{
             attribute mem:operation { $operation },
...
declare function mem:all-nodes-from-same-doc($nodes as node()*,$parent-node as node()) as empty-sequence() {
  ...
  then ()
    ...
    error(xs:QName("mem:MIXEDSOURCES"), "The nodes to change are coming from multiple sources",$nodes)
};
...
as node()*
{
  mem:all-nodes-from-same-doc($nodes-to-modify,root($nodes-to-modify[1])),
  mem:safe-copy(mem:process((), $nodes-to-modify, $new-nodes, $operation, ()))
};
...
as node()*
{
  mem:process(
    ...
    $new-nodes,
    $operation,
    mem:find-ancestor-intersect($outermost-nodes-to-modify, 1, ())
        except
    (if (exists($root-node))
...
function mem:process(
  ...
  $common-parent as node()?,
  $merging-nodes as node()*,
  $trees as element(mem:trees))
as node()*
{
  if (exists($common-parent))
  then
    mem:process-ancestors(
      $transaction-map,
      ...
      $new-nodes,
      mem:reconstruct-node-with-additional-modifications(
        $transaction-map,
        $common-parent,
        mem:place-trees(
...
as node()*
{
  let $mod-node-id-qn := mem:generate-id-qn($nodes-to-modify[1])
    ...
    mem:build-new-xml(
      ...
      typeswitch ($operations)
       ...
       default return
         ...
         @operation,
      typeswitch ($new-node)
       case element(mem:modifier-nodes)* return $new-node[*[node-name(.) eq $node-to-modify-id-qn]]
       default return
         <mem:modifier-nodes>{
             attribute mem:operation { $operations },
             $new-node
           }</mem:modifier-nodes>))
};
...
declare %private
function mem:find-ancestor-intersect(
  $items as node()*,
...
as node()*
{
  ...
  then $merging-nodes
    ...
    let $nodes-inbetween := nodeops:inbetween($merging-nodes, $previous-node-to-modify, $node-to-modify)
    return
      (
        $nodes-inbetween,
...
declare %private
function mem:process-ancestors(
  $transaction-map as map(*)?,
  $ancestors as node()*,
...
declare %private
function mem:reconstruct-node-with-additional-modifications(
  $transaction-map as map(*)?,
  $node as node(),
  ...
  $carry-over-attributes as xs:boolean)
{
  ...
  then
    mem:process-subtree(
      $transaction-map,
      (),
      mem:reconstruct-node($node,$ordered-content,$nodes-to-modify,$carry-over-attributes),
      mem:generate-id-qn($node),
      $new-node,
      $operations,
      ()
    )
...
declare
function mem:id-wrapper($node as node())
{
  element {mem:generate-id-qn($node)} {()}
};

(: Generate QName from node :)
declare %private
function mem:generate-id-qn($node as node())
{
  QName(
      "http://maxdewpoint.blogspot.com/memory-operations",
      concat("_", mem:generate-id($node)))
};

(: Generate an id unique to a node in memory. Right now using fn:generate-id. :)
declare %private
function mem:generate-id($node as node())
{
  generate-id($node)
};
...
declare
function mem:function-key($function as function(*))
{
...
declare %private
function mem:build-new-xml(
  $transaction-map as map(*)?,
  $node as node(),
  $operations as xs:string*,
  $modifier-nodes as element(mem:modifier-nodes)*)
{
  mem:build-new-xml(
    $transaction-map,
    $node,
    mem:weighed-operations(distinct-values($operations)),
    $modifier-nodes,
    ()
  )
};
  ...
  $modifying-node as node()?)
{
  ...
  then $nodes
    ...
    let $mod-nodes as node()* :=
      let $modifier-nodes :=
            if ($last-in-wins)
            then ($modifier-nodes[@mem:operation eq $operation])[1]
            else if ($reverse-mod-nodes) 
            then reverse($modifier-nodes[@mem:operation eq $operation])
            else $modifier-nodes[@mem:operation eq $operation]      
      return
        ($modifier-nodes ! @node()[empty(self::attribute(mem:operation))],
         ($modifier-nodes ! node()[empty(self::mem:*)])[namespace-uri(.)!="http://maxdewpoint.blogspot.com/memory-operations"])
    let $new-nodes :=
      switch ($operation)
...
declare %private
function mem:weighed-operations(
...
declare %private
function mem:safe-copy(
  $node as node())
as node()? {
  mem:reconstruct-node($node,$node/*,(),fn:true())
};
