...
declare function local:credentials-from-session() as xs:string* {
    (session:get-attribute("metis.user"), session:get-attribute("metis.password"))
};
...
declare function local:set-credentials($user as xs:string, $password as xs:string?) as element()+ {
    session:set-attribute("metis.user", $user), 
    session:set-attribute("metis.password", $password),
...
(: Resource paths starting with $shared are loaded from the shared-resources app :)
    ...
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <cache-control cache="max-age=600, public"/>
    </dispatch>
