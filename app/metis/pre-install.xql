...
return
if (starts-with($config:metis-config/configuration/root/string(), "/db")) 
then (: data collection not in app-root :)
    (
    util:log-app('DEBUG', 'metis', fn:concat("Config: Creating data collection '", $config:metis-root, "'...")),
    for $col in $config:metis-data-collections return
    (
        local:mkcol($config:metis-root, local:strip-prefix($col, fn:concat($config:metis-root, "/")))
    ),
    util:log-app('DEBUG', 'metis', "...Config: Uploading initial data..."),
    xdb:store-files-from-pattern($config:metis-users,  $dir, "data/users/*.xml"),
    xdb:store-files-from-pattern($config:metis-templs, $dir, "data/templates/*.xml"),
    util:log-app('DEBUG', 'metis', "...Config: Done Uploading inital data."),
    util:log-app('DEBUG', 'metis', "...Config: Fixing data permissions."),
    dbutil:scan(xs:anyURI($config:metis-root), function($collection, $resource) {
        if ($resource)
    ...
    util:log-app('DEBUG', 'metis', "Config: Done.")
    )
    else ()
};
...
if (sm:group-exists($config:admin-group)) then
    ...
    sm:create-group($config:admin-group, "metis administrator group"),
