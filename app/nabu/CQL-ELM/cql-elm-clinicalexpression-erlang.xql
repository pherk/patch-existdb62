...
let $xsd := doc("/db/apps/nabu/CQL-ELM/clinicalexpression.xsd")
let $ctypes := $xsd/xs:schema/xs:complexType
return
<output>
{
    string-join((
         "-ifndef(cql_elm_clinicalexpression)."
        , "%%%"
...
return
    let $ct := $type
    let $annots := tokenize($ct/xs:annotation/xs:documentation,'\n')
