...
{
    let $cname := letter:selectName($contact)
    let $name := switch ($contact/fhir:gender/@value)
        case 'male' return concat("Herr ", letter:formatFHIRName($cname))
        case 'female'  return concat("Frau ", letter:formatFHIRName($cname))
        default return letter:formatFHIRName($cname)
    return
...
declare function letter:selectName($contact) as element(fhir:name)
{
    if (count($contact/fhir:name)>1)
    then if ($contact/fhir:name[fhir:use/@value='official'])
        then $contact/fhir:name[fhir:use/@value='official']
        else head($contact/fhir:name)
    else $contact/fhir:name
};
...
{
    let $cname := letter:selectName($contact)
    let $fname := switch ($contact/fhir:gender/@value)  
        case 'male' return concat("Sehr geehrter Herr ", letter:formatFHIRName($cname))
        case 'female'  return concat("Sehr geehrte Frau ", letter:formatFHIRName($cname))
        default return concat('Sehr geehrte Herr/Frau ', letter:formatFHIRName($cname))
    return 
        $fname
};
            ...
            <list rend="bulleted">
            {
                let $olist := 
                    ...
                    (
                        ...
                        , <item xmlns="http://www.tei-c.org/ns/1.0"><list rend="bulleted">
                        {
                            ...
                            let $reasonCode := letter:mapCalendarLabel($oa/fhir:type/fhir:coding[fhir:system/@value="#encounter-type"]/fhir:display/@value)
                ...
                let $nlist :=
                    ...
                    (
                        ...
                        {
                            for $na in $newapps
                            let $reasonCode := letter:mapCalendarLabel($na/fhir:type/fhir:coding[fhir:system/@value="#encounter-type"]/fhir:display/@value)
