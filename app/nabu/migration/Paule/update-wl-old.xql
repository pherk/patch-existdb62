...
let $lll := util:log-system-out(count($aos2511))
...
return
    if (count($old)=1)
        then
            ...
            let $actor  := r-practitioner:practitionerByIdentifier($walias, 'kikl-spz', 'u-admin','true')
            let $aref   := try {
                    concat('metis/practitioners/', $actor/fhir:id/@value)
            } catch * {
                util:log-system-out($walias)
            }
            ...
            let $details := for $d in $old/fhir:detail
                      let $dbase := $d/fhir:*[not(self::fhir:actor)]
                      return
