...
let $dupimport := true()
let $import-date := "2024-04-02"
let $import-folder := "Befunde2024"

(: below is nothing to change!!! :)
let $groups      := ('Arzt','Bayley3','Ergotherapie','Logopaedie','Orthoptik','Physiotherapie','Psychologie','Sozialarbeit','Verwaltung')
...
for $group in $groups
let $letters := collection(concat($import-base, "/", $import-folder, '/', $group))
let $ass     := 
...
:)
for $l in xmldb:find-last-modified-since($letters,xs:dateTime(concat($import-date, "T00:00:00")))
...
try {
    ...
    let $date := helper:validdate($lt//tei:opener//tei:dateline)
    return
        if ($ss/*:error or $rec/*:error or $as/*:error or $date/*:error)
        then
